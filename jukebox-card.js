class JukeboxCard extends HTMLElement {
    ...
    buildSpeakerSwitches(hass) {
        ...
        this.config.entities.forEach(entityObj => {

            let entityId = entityObj.media_player;
            ...
            this._tabs.appendChild(this.buildSpeakerSwitch(entityObj, hass));
    ...
    buildVolumeSlider() {
        ...
        this._hassObservers.push(hass => {
            if (!this._selectedSpeaker.media_player || !hass.states[this._selectedSpeaker.media_player]) {
                return;
            }
            const speakerState = hass.states[this._selectedSpeaker.media_player].attributes;
            ...
            if (hass.states[this._selectedSpeaker.media_player].state === 'playing') {
    ...
    onSpeakerSelect(entityId) {
        ...
        let speaker = this.config.entities.find(function(item, index) {
            if (item.media_player === entityId)  {
                return item;
            }
        });
    ...
    onMuteUnmute(e) {
        this.hass.callService('media_player', 'volume_mute', {
            entity_id: this._selectedSpeaker.media_player,
    ...
    updateStationSwitchStates(hass) {
        ...
        if (hass.states[selectedSpeaker.media_player] && hass.states[selectedSpeaker.media_player].state === 'playing') {
            playingUrl = hass.states[selectedSpeaker.media_player].attributes.media_content_id;
        }
    ...
    onStationSelect(e) {
        ...
        let mediaPath = e.currentTarget.stationUrl;
        let contentType = 'music'

        console.log(e.currentTarget);
        
        if (this._selectedSpeaker.type === 'sonos') {
            mediaPath = 'x-rincon-mp3radio:'+mediaPath.substring(mediaPath.lastIndexOf('//'));
            if (mediaPath.endsWith('m3u') || mediaPath.endsWith('m3u8')) {
                contentType = 'music';
            }
        } else if (this._selectedSpeaker.type === 'yamaha') {
            mediaPath = 'Bookmarks>Internet>'+e.currentTarget.textContent;
            contentType = 'NET RADIO';
        }

        console.log('Start playback on : ' + this._selectedSpeaker.media_player + ' / ' + mediaPath + ' / ' + contentType);
    ...
    buildSpeakerSwitch(entityObj, hass) {
        let entityId = entityObj.media_player;
        const entity = hass.states[entityId];

        const btn = document.createElement('paper-tab');
        btn.entityId = entityId;
        btn.deviceType = entityObj.type;
        btn.title = entityObj.type + ' device';

        if (entityObj.name) {
            btn.innerText = entityObj.name;
        } else {
